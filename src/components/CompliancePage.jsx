...
const MATRIX_ROWS = [
  ...
  },
  {
    ...
    modalHow: 'The compliance pack installs 20+ pre-built Kibana Security detection rules organized into 12 category-specific rule sets (A–L). Each rule set maps to a specific Appendix B event category and MITRE ATT&CK technique, enabling immediate CEM coverage from the moment rules are enabled. Rules are ECS-normalized and work across all Appendix B log sources out of the box. The Category K meta-rules provide automated compliance monitoring — alerting when any category stops receiving events, giving teams early warning before CEM attestation breaks. Category L extends coverage to passively-observed OT/IoT devices that never run an agent.',
  ...
  },
  {
    ...
    capDesc: '7 custom ML jobs (DNS entropy, five compliance-element monitors, new network device) plus 7 Elastic Security ML module jobs prefix-installed for behavioral detection.',
    modalHow: 'The pack runs two ML job families. Seven custom jobs ship with the pack: DNS entropy anomalies (Categories B/G), the five compliance-element monitors (asset-coverage drops, ingestion-rate dips, rule silence, ILM/retention anomalies, hash-coverage gaps), and new or rarely-seen devices per network segment (Element 1 / Appendix A HWAM, backing Category L). Seven Elastic Security ML module jobs are prefix-installed with the m_26_14_ job-id prefix to power the behavioral detection rules: authentication anomalies and rare IP access patterns (Category A), suspicious login activity, host-went-silent, rare process execution on Linux and Windows hosts, and rare destination countries (Category B). Each job includes its datafeed and builds behavioral baselines automatically from historical log data. ML-generated anomaly records constitute Appendix B Category I events.',
